import type { Middleware, MiddlewareOptions } from "../middleware/index.types";
...
export function shouldRunMiddleware(
	...
	if (
		...
		!config.includes.some((p) => p.match.exec(path) && methodMatch(p, method))
	) {
		return false;
	}
	...
	if (
		...
		config.excludes.some((p) => p.match.exec(path) && methodMatch(p, method))
	) {
		return false;
	}
